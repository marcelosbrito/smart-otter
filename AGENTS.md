...
- Main specs: `openspec/specs/`; active changes: `openspec/changes/`; archived: `openspec/changes/archive/`. ADRs in root `adr/` (active) and `openspec/adr/` (historical).
...
- **Database**: Supabase PostgreSQL. Schema in `src/lib/db/types.ts`. Tables: `users`, `favorites`, `knowledge_cache`. RLS enabled, service-role policies for API routes.
  - Server-side: `createSupabaseServerClient()` from `src/lib/db/supabase-server.ts` — singleton cached in module (`cachedClient`). Uses `SUPABASE_SERVICE_ROLE_KEY`. Exports a stub when credentials are missing (returns `{ data: null, error }` chains). Once cached (even as a stub), changing `.env.local` requires a dev server restart.
  - Browser-side: exported `supabase` from `src/lib/db/supabase-client.ts` — uses `@supabase/ssr`'s `createBrowserClient`, falls back to `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` if `SUPABASE_ANON_KEY` is unset.
