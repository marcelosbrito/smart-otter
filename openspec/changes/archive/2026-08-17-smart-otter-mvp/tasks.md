...
- [x] 2.1 Create Clerk application at dashboard.clerk.dev and obtain publishable/secret keys (requires manual action)
...
- [x] 2.5 Verify email login, Google OAuth, and GitHub OAuth flows work in development mode (requires real Clerk keys from step 2.1)
...
- [x] 3.5 Write unit tests for each provider implementation verifying interface contract compliance
...
- [x] 9.1 Create server actions for `saveFavorite`, `removeFavorite`, and `getFavorites` that interact with the SQLite repository layer
- [x] 9.2 Implement `app/favorites/page.tsx` displaying resources grouped by profession using shadcn Card components
- [x] 9.3 Add the Save Favorite button on resource cards in search results (visible only when authenticated)
- [x] 9.4 Implement unauthenticated user redirect to sign-in when attempting to save a favorite, with return navigation after login
- [x] 9.5 Write unit tests for server actions verifying database operations and authentication checks

## 10. Integration, Polish, and Validation

- [x] 10.1 Run `openspec validate smart-otter-mvp --type change --strict` to verify all spec scenarios are satisfied
- [x] 10.2 Add responsive design adjustments for mobile/tablet breakpoints using Tailwind CSS media queries
- [x] 10.3 Implement proper error boundaries and fallback UI for API failures throughout the application
- [x] 10.4 Add basic accessibility improvements: ARIA labels, keyboard navigation, focus management in modals/dialogs
- [x] 10.5 Create a production build (`npm run build`) and verify no TypeScript errors or missing environment variables
